...
class IOContext(object):
    ...
    download_limits = attr.ib(default=attr.Factory(
        lambda: DownloadLimits()))
    excludechars = attr.ib(default='*/|')
    proxy = attr.ib(default=None)
    rtmpdump_binary = attr.ib(default=None)
    hds_binary = attr.ib(default=attr.Factory(
        lambda: ['php', resource_filename(__name__, 'AdobeHDS.php')]
    ))
